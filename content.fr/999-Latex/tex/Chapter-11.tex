...
\begin{tcolorbox}[colback=CSCOLORPARTTWO!50!white,colframe=CSCOLORPARTTWO,title=Les Concepts Clés]
    \begin{description}
        \item[ACTION] Tout ce qu'un personnage fait qui soit significatif---frapper un adversaire, sauter par-dessus un torrent, activer un appareil, utiliser un pouvoir spécial, et ainsi de suite. Chaque personnage peut faire une action en un round.
        \item[PERSONNAGE] Toute créature qui peut être interprétée, que ce soit un personnage-jour (PJ) interprété par un joueur, ou un personnage-non-joueur (PNJ) interprété par la Meneuse. Dans le Cypher System, les créature bizarres, les machines conscientes, et l'énergie vivante, peuvent toutes être des personnage.
        \item[\uppercase{Difficulté}] Une mesure d'à quel point il est facile d'accomplir une tâche. La difficulté est évaluée sur une échelle de 1 (le plus facile) à 10 (le plus difficile). Modifier la difficulté pour rendre une tâche plus difficile est appelé "désavantager". La rendre plus facile est appelé "faciliter". Un changement de la difficulté est mesuré en crans. La difficulté est souvent égale au niveau, donc ouvrir une porte fermée de niveau 3 aura probablement une difficulté de 3.
        \item[FACILITER] Une diminution de la difficulté d'une tâche, d'habitude d'un cran. Si il n'est pas indiqué de combien de cran une tâche est facilitée, alors la difficulté est réduite de un cran.
        \item[EFFORT] Dépenser des points d'une Réserve de stat pour réduire la difficulté d'une tâche. Un PJ peut décider si, oui ou non, il applique de l'effort pendant son tour avant que le jet de dé soit effecté. Les PNJs n'applique jamais d'Effort.
        \item[ENTRAVER] Une augmentation de la difficulté de la tâche, d'habitude d'un cran. Si il n'est pas indiqué de combien de cran une tâche est désavantagée, alors la difficulté est augmentée de un cran.
        \item[INAPTITUDE] L'opposé d'être entraîné---vous êtes désavantagé à achque fois que vous tentez d'accomplir une tâche pour laquelle vous êtes inapte. Si vous devenez entraîné à cette tâche, l'entrainement et l'inaptitude se compensent l'un l'autre et vous avez maintenant de la pratique dans la tâche.
        \item[NIVEAU] Une façon de mesurer la force, la difficulté, la puissance, le défi de quelque chose dans le jeu. Tout, dans le jeu, a un niveau. Les PNJs, et les objets ont un niveau pour déterminer la difficulté de toute tâche les concernant. Par exemple, le niveau d'un adversaire définit à quel point il est difficile à toucher ou à éviter dans un combat. Le niveau d'une porte indique à quel point il est difficile de la briser. Le niveau d'une serrure définit la difficulté de la crocheter. Les niveaux sont définis sur une échelle de 1 (le plus facile) à 10 (le plus difficile). Les rangs des personnages sont un petit peu comme les niveaux mais ne cont que de 1 à 6 et mécaniquement fonctionne de maière très différente des niveaux---par exemple, le rang d'un personnage ne détermine pas la difficulté d'une tâche.
        \item[PRATIQUE] La capacité normale, non-altérée, d'utiliser une compétence---non-entraîné, non-spécialisé, sans inaptitude. Votre type détermine quelles sont vos compétences en arme pour lesquelles vous avez de la pratique; si vous n'avez pas de pratique dans un type d'arme, vous être inapte avec lui.
        \item[\uppercase{Jet de dé}] Un jet de `d20` fait par un PJ pour déterminer si une action est un succès. Bien que le jeu utilise occasionellement d'autres dés, quand le texte fait simplement fait référence à "un jet de dé" c'est toujours un jet de `d20`.
        \item[ROUND] Une durée entre 5 et 10 secondes de longueur. Il y a environ 10 rounds dans une minute. Quand il est très important de suivre le temps de manière précise, utilisez le round. En pratique, c'est la durée que prend une action dans le jeu, mais comme tout le monde agit plus ou moins de manière simultanée, tous les personnages peuvent effectuer une action chaque round.
        \item[\uppercase{Spécialisé}] Avoir un niveau exceptionnel dans la compétence pour une tâche. Etre spécialisé facilite la tâche de deux crans. Donc, si vous êtes spécialisé dans l'escalade, toutes les tâches d'escalade sont facilités de deux crans.
        ...
        \item[\uppercase{Tâche}] Toute action qu'un PJ essaie d'accomplir. La Meneuse détermine la difficulté d'une tâche. En général, une tâche est quelque chose que vous faîtes, et une action est votre accomplissement de cette tâche, mais dans la plupart des cas ils signifient la même chose.
        \item[\uppercase{Entraîné}] Avoir un niveau raisonable dans la compétence pour une tâche. Être entraîné facilite la tâche. Par exemple, si vous êtes entraîné en escalade, toutes les tâches d'escalade sont facilités pour vous. Si vous devenez très entraîné à cette tâche, vous devenez spécialisé à la place d'entraîné. Vous n'avez pas besoin d'être entrapiné pour tenter d'accompir une tâche.
        \item[TOUR] La partie du round pendant laquelle le personnage ou la créature réalise ses actions. Par exemple, si un Guerrier et un Adepte combattent un orc, le Guerrier fait une action pendant son tour, l'Adepte fait une action pendant son tour, et l'orc fait son action pendant son tour. Certaines capacités ou effets durent seulement pendant un tour, ou se termine quand le prochain tour démarre.
    \end{description}
\end{tcolorbox}
...
\subsection*{Determiner la Difficulté de la Tâche}
...
\textbrf{Niveaux Gratuit d'Effort:} Il y a quelques capacités qui peuvent vous donner un niveau d'Effort gratuit (en général elles vous demandent d'appliquer au moins un niveau d'Effort pour la tâche). En pratique, vous gagnez un niveau d'Effort supplémentaire en plus de ce que pourquoi vous avez payé. Ce niveau d'Effort gratuit peut dépasser la limite de l'Effort pour votre personnage, mais pas la lmite des six crans d'Effort maximum pour faciliter une tâche.
...
\begin{tcolorbox}[title=Regardons de plus près les situations qui n'impliquent pas de PJ]
\end{tcolorbox}
\subsection*{A Closer Look at Situations That Don't Involve PC}
...
\subsection*{Damage From Hazards}
...
\begin{table*}[t!]
    ...
    \centering
        \begin{tabular}{ l l l } 
        \multicolumn{3}{ l }{\Large \textcolor{CSCOLORPARTTWO}{Damage From Hazards}} \\
...
\subsection*{Radiation Belt/Solar Flare}
...
\subsection*{Asteroid/Debris Field}
...
\subsection*{The Effects of Taking Damage}
...
\begin{description}
    ...
    \item[Impaired] is a wounded or injured state. When an impaired character applies Effort, it costs 1 extra point per level applied. For example, applying one level of Effort costs 4 points instead of 3, and applying two levels of Effort costs 7 points instead of 5.
...
\item[Debilitated] is a critically injured state. A debilitated character may not take any actions other than to move (probably crawl) no more than an immediate distance. If a debilitated character's Speed Pool is 0, they can't move at all. When a debilitated PC takes enough damage to reduce a stat Pool to 0, they are dead.
\item[Dead] is dead.
\end{description}
...
\subsection*{Recovering Points in a Pool}
...
\subsection*{Restoring the Damage Track}
...
\subsection*{NPCs and Special Damage}
...
\subsection*{Attack Modifiers And Special Situations}
...
\subsection*{SPECIAL SITUATION: COMBAT BETWEEN NPCs}
...
\subsection*{SPECIAL SITUATION: COMBAT BETWEEN PCs}
...
\subsection*{SPECIAL SITUATION: AREA ATTACKS}
...
\subsection*{SPECIAL SITUATION: ATTACKING OBJECTS}
...
\section*{Action: Activate a Special Ability}
...
\subsection*{Special Situation: A Chase}
...
\section*{Action: Do Something Else}
...
\subsection*{Cooperative Actions}
...
\subsection*{Crafting, Building, And Repairing}
...
\subsection*{Crafting Difficulty and Time}
...
\subsection*{Interacting With Creatures}
...
\subsection*{Looking Or Listening}
...
\subsection*{Moving A Heavy Object}
...
\subsection*{Operating Or Disabling A Device, Or Picking A Lock}
...
\subsection*{Riding Or Piloting Difficulty}
...
\begin{table*}[h]
    ...
    %\centering
        ...
        \textbf{Difficulté} & \textbf{Maneuvre} \\ [0.5ex]
        \rowcolor{CSCOLORPARTTWO!50}
    0          & Riding \\
    \rowcolor{CSCOLORPARTTWO!20}
...
\subsection*{Understanding, Identifying, Or Remembering}
...
\section*{Breathing Life Into Followers}
...
\begin{itemize}
    \item A firebrand diplomat able to convince an enemy horde to back down.
    \item A veteran commander whose presence bolsters the entire community's military might.
    \item A genius medic who invigorates everyone with their healing techniques.
    \item An imaginative architect whose works both beautify and defend the city.
    \item A tricky spy whose intelligence on enemy movements is invaluable.
\end{itemize}
